    ...
    public Claims getClaimsFromGoogleOauthIdToken(String idToken) {
        try {
            ResponseEntity<Map<String, Object>> response = restTemplate.exchange(
                    "https://www.googleapis.com/oauth2/v3/certs", HttpMethod.GET, null,
            ...
            List<?> keysRaw = (List<?>) body.get("keys");
            List<Map<String, Object>> keys = keysRaw.stream()
                    .filter(item -> item instanceof Map)
                    .map(item -> {
                        if (item instanceof Map<?, ?>) {
                            Map<?, ?> map = (Map<?, ?>) item;
                            // Ensure all keys are Strings and all values are Objects
                            boolean isStringObjectMap = map.keySet().stream().allMatch(k -> k instanceof String);
                        ...
                        throw new IllegalArgumentException("Invalid key format in JWKs.");
                    })
                    ...
                    if (kid.equals(key.get("kid"))) {
                        try {
                            ...
                            return KeyFactory.getInstance(
                                    key.get("kty").toString()).generatePublic(rsaPublicKeySpec);
                        } catch (Exception e) {
